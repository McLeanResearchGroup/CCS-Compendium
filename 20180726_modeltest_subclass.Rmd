...
#      fit <- nls(y ~ b+(top-b)/(1+(x/xmid)^scal), data = xy,
...
#      fit <- nls(y ~ b+(top-b)/(1+10^(((xmid+(1/scal)*log(2^(1/s)-1))-x)*scal))^s, data = xy,
#      fit <- nls(y ~ b+(top-b)/(1+(x/xmid)^H)^s, data = xy,
...
for(i in seq_along(data)){
  if(!is.na(AICs[i, 5])) {
    ...
    fitteddata[[name]] <- fitteddata[[name]][, c(1, 4, 9, 8)]
  } else{}
}
...
for(i in seq_along(fitteddata)){
  ...
  ci <- CI(stdErr, y, fitteddata[[i]][, 6])
  pi <- PI(stdErr, y, fitteddata[[i]][, 6])
  fitteddata[[i]] <- cbind(fitteddata[[i]], ci, pi, stdErr)
  colnames(fitteddata[[i]]) <- c('Compound', 'm/z', 'CCS/z', 'rsd', 'X curve', 'Y curve', 
                                 'CI lo', 'CI hi', 'PI lo', 'PI hi', 'std error')
}
...
for(i in names(fitteddata)){
  write.csv(fitteddata[[i]], paste0("~/Box Sync/20171218JAP_iceberg/R-output/SubClassTrends_R/", i,".csv"))
}
#  write.csv(fitteddata[[i]], paste0("~/Box Sync/R_Scripts&Data/20171218JAP_iceberg/R-output/SubClassTrends_R/", i,".csv"))

write.csv(powertable, "~/Box Sync/20171218JAP_iceberg/R-output/subclass_Powerfit_equations.csv")
write.csv(fourtable, "~/Box Sync/20171218JAP_iceberg/R-output/subclass_4Psigfit_equations.csv")
write.csv(fivetable, "~/Box Sync/20171218JAP_iceberg/R-output/subclass_5Psigfit_equations.csv")
#R_Scripts&Data/
